...
class Model(metaclass=ModelMeta):
    ...
    def save(self, db_conn: Connection):
        field_names = self.get_field_names()
        field_values = tuple(getattr(self, field_name) for field_name in field_names)

        field_names_string = ', '.join(field_names)
        values_string = ', '.join(len(self._fields) * '?')

        query = f'INSERT OR REPLACE INTO {self.Meta.table_name} ({field_names_string}) VALUES ({values_string})'
    ...
    @classmethod
    def ensure_table(cls, db_conn: Connection):
